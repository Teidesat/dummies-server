...
def random_modify(string: bytes) -> bytes:
    ...
    if random.randint(1, 3) % 3 == 0:
        ...
        mutation_indices = random.sample(
            range(len(string_array)), min(num_mutations, len(string_array))
        )
...
def show_help_message():
    ...
    print(
        "Provides a way to test the receiver's server by sending packets of information in the same format as the firmware"
    )
